...
function random_int_between(a, b){
    var v = Math.floor(Math.random() * (b-a) + a);
    return v;
}
...
function extracted_parameters(){
    ...
    /* note the L2refspec is a flat spec. value at each octave should be greater than the max of specVariation. 
    This is used to generate a proper random number for between a and b as shown in random_int_between(a,b)
    */
    var parameters = {L2refspec:[12,12,12,12,12], specVariation:[6, 5, 6, 11, 11], cNormalised:[-21,-14,-8,-5,-4], ctrNormalised:[-14,-10,-7,-4,-6]} ;
   return parameters;
}
...
*/
    ...
    if (C50perRangeVent>Ctr50perRangeVent){
      var recommendVentCtr = "Dne,w+Ctr";
      var recommendVentC = "";
    }
    else{
      var recommendVentCtr = "";
      var recommendVentC = "Dne,w+C";
    }
  ...
  /* transfer to column array
  var RwCCol = transposeArray([RwC]);
  var RwCtrCol = transposeArray([RwCtr]);
  return RwC;
  */
}
...
function newChart() {
  ...
  var chart = new google.visualization.Line(); //PieChart(document.getElementById('chart_div'));
  var data = [1,2,3,4,5];
  chart.draw(data);
